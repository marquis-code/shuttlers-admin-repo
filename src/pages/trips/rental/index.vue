<template>
	<main class="">
		<Table
			...
			:headers="tableFields"
			:table-data="rentalList"
			:has-index="true"
			:page="page"
			:has-options="true"
			:option="(data) => $router.push(`/trips/rental/${data.id}`)"
		>
			<template #header>
				<TableFilter
					:filter-type="{
				...
				<div class="bg-white border-x border-gray-200">
					<div class="flex justify-end items-end p-3 gap-4">
						<div class="flex items-center gap-x-2">
							<label class="font-medium text-base my-0"> Status: </label>
						...
						<div class="flex items-center gap-x-2">
							<label class="font-medium text-base my-0"> Booking Status: </label>
			...
			<template #item="{ item }">
				<div v-if="item.pickup_address">
					<RouteDescription
						:pickup="item.data.pickup_address"
						:destination="item.data.dropoff_address"
					/>
				</div>
				...
				<div v-if="item.updated_at">
					{{ item.data.user?.phone ?? item.data?.user?.corporate_phone }}
				</div>
