<template>
	<div class="">
		<div class="flex ">
			<div class="w-4/12 bg-white border-r p-6 space-y-6 sticky top-4 h-[1000px] overflow-y-auto">
				...
				<div class="space-y-3">
					...
					<div class="flex items-center flex-wrap gap-x-3">
						<div v-for="item in form.selected_dates" :key="item" class="flex px-3 py-2 items-center gap-x-2 rounded-lg bg-blue-100 border-blue-700">
							<p class="text-gray-900 text-sm">
								{{ item }}
							</p><span class="cursor-pointer text-rose-500 font-medium" @click="removeSelected(item)">X</span>
						</div>
					</div>
					<button class="flex items-center gap-x-2 bg-black text-white text-sm px-3 py-2 rounded-md" @click="toggleDatePicker">
						<img src="@/assets/icons/source/white-plus.svg" alt="plus">Add Date
					</button>
					<div v-if="showDatePicker">
						<InputDateInput id="startDate" v-model="form.selected_date" class="font-light" placeholder="Filter by date" @update:model-value="handleSelectedDate" />
					</div>
				</div>
				<div v-if="selectedRouteAvailability === 'selected_days'" class="flex items-center flex-wrap gap-x-3">
				...
				<div class="space-y-3">
					<div>
						...
						<div>
							<label>Time</label>
							<div><InputTimeInput v-model="form.itineraryTime" /></div>
						</div>
					</div>
					<div>
						<div>
							<label>Fare</label>
							<div><input class="border w-full py-3 outline-none rounded-md input-field px-3" type="number"></div>
						</div>
					</div>
				</div>
				<div>
			...
			<div class="w-8/12">
				<MapDisplay :start-point="form.startLocation" :end-point="form.endLocation" :stops="form.stopPoints" :props-loading="loading" />
			</div>
		</div>
	</div>
</template>
...
const selectedRouteAvailability = ref('everyday')
  ...
  const addStopPoint = () => {
	if (Object.keys(form.startLocation).length > 0) {
		...
		useAlert().openAlert({ type: 'ERROR', msg: 'You need to add a start point before adding a stop point.' })
	}
  }
...
const selectedAdditionalEndPoint = (val) => {
	...
	const handleSelectedDay = ($event: any) => {
		form.route_availability_days.push($event.target.value)
		if (form.route_availability_days.length === 7) {
	...
	const handleSelectedDate = (val:string) => {
		if (!form.selected_dates.includes(val)) {
			...
			useAlert().openAlert({ type: 'ERROR', msg: `${val} has already been added.` })
		}
	}
