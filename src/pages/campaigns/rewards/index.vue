<template>
	<main class="space-y-10">
		...
		<div class="text-center bg-black h-12 text-sm text-white flex justify-center items-center">
			Current pilot point rate: 1 point ~ {{ loading_points_rate ? 'loading...' : convertToCurrency(pointsRateObject?.value) }}
		</div>
		<div>
					...
					<Table :loading="loadingPointsList" :headers="tableFields" :table-data="computedPointsList">
						...
						<template #item="{ item }">
							...
							<div v-if="item.action" class="flex items-center gap-x-4 w-full">
								...
								<div class="w-full">
									<button v-if="lookupTable[item.data.id]" class="text-black font-medium" @click="lookupTable[item.data.id] = false">
										Cancel
									</button>
								</div>
							</div>
