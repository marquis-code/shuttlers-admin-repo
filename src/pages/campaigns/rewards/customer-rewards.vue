		...
		<div>
			<div class="container mx-auto space-y-3">
				<div class="lg:flex justify-between items-center px-4 py-2 border rounded-lg space-y-4 lg:space-y-0">
					<div class="flex border h-10 border-gray-300 rounded-lg">
						<button :class="[activeTab === 'leaderboard' ? 'bg-gray-200 text-[#364152]' : 'bg-white text-[#101828]']" class="font-[500] w-full lg:w-max rounded-l-lg text-center flex justify-center items-center leading-[28px] text-xs px-3 py-2" @click="activeTab = 'leaderboard'">
							Customers Leaderboard
						</button>
						<p class="h-2 border-gray-500" />
						<button :class="[activeTab === 'list' ? 'bg-gray-200 text-[#364152]' : 'bg-white text-[#101828]']" class="font-[500] w-full lg:w-max leading-[28px] text-center flex justify-center items-center text-xs  rounded-r-lg px-3 py-2" @click="activeTab = 'list'">
							Reward list
						</button>
					</div>
				...
				<div v-if="activeTab === 'leaderboard'" class="space-y-6">
					...
					<div class="overflow-x-auto rounded-lg border-[0.4px]">
						<table class="min-w-full text-xs">
							...
							<thead class="bg-white">
								<tr class="text-left text-gray-900">
									<th v-for="(item, index) in ['S/N', 'CUSTOMER', 'TOTAL POINT', 'CURRENT POINT']" :key="index" class="px-3 py-4">
										{{ item }}
									</th>
								</tr>
							</thead>
							<tbody class="">
								<tr v-for="(item, index) in leaderboardList" :key="index" class="border-[0.4px] border-opacity-20 odd:bg-white even:bg-slate-50 text-gray-950">
									...
									<td class="px-3 py-6">
										<NuxtLink :to="`/campaigns/rewards/${index}/reward-history`" class="font-medium underline text-[#4848ED]">
											{{ item.customer }}
										</NuxtLink>
									</td>
									<td class="px-3 py-6">
