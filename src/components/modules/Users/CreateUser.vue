			...
			<form class="flex flex-col gap-4 px-6 py-12" @submit.prevent="type === 'new' ? createUser() : editUser(id)">
				<div class="grid lg:grid-cols-2 gap-8">
					<div class="field relative">
						...
						<input id="First" v-model="createForm.fname.value" name="first_name" type="text" class="input-field" required>
					</div>

					<div class="field relative">
						<label for="Last">Last name</label>
						<input id="Last" v-model="createForm.lname.value" name="last_name" type="text" class="input-field" required>
					</div>
				</div>
				<div class="grid lg:grid-cols-2 gap-8">
					<div class="field relative">
						<label for="Email">Email address</label>
						<input id="Email" v-model="createForm.email.value" name="email_address" type="email" class="input-field" required>
					</div>

					<div class="field relative">
						<label for="Phone">Phone</label>
						<input id="Phone" v-model="createForm.phone.value" name="phone" type="tel" class="input-field" required>
					</div>
				</div>
				<div class="grid lg:grid-cols-2 gap-8">
					<div class="field">
						<label for="Date">Date of Birth</label>
						<InputDateInput id="Date" v-model="createForm.dob.value" name="date_of_birth" :disabled-date="preventDisableDate" class="font-light" placeholder="Filter by date" />
					</div>
					<div class="field">
				...
				<div class="grid lg:grid-cols-2 gap-8">
					<div v-if="type === 'new'" class="field relative">
						...
						<input id="password" v-model="createForm.password.value" name="password" autocomplete="true" :type="showPassword ? 'text' : 'Password'" class="input-field" required>
					...
					<div class="field relative">
						...
						<InputMultiSelectCompanies name="company_name" obj-key="id" :value="selectedUser.company" @updated="createForm.corporate_id.value = $event" />
					</div>
				</div>
