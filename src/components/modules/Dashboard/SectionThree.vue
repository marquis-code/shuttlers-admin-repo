		...
		<div class="p-5 flex items-center gap-x-3">
			...
			<div v-else class="rounded-md border bg-white shadow-sm space-y-4 p-6 lg:w-4/12">
				<div class="flex justify-between items-center">
					<div>
						...
						<p>
							...
							>/{{ tripRatingData?.max_value }}
						</p>
					</div>
					<div>
						<img
							src="@/assets/images/rating-star.jpg"
							alt=""
				...
				<p class="flex items-center space-x-1">
					...
					>{{ tripRatingData.count }} ratings in total</span
					>
				</p>
			</div>
			<Skeleton v-if="loadingRatingByDate" height="150px" width="400px" />
			<div v-if="loadingRatingByDate === false && isFilterUpdated === true" class="rounded-md border bg-white shadow-sm space-y-4 p-6 lg:w-6/12">
				<div class="flex justify-between items-center">
					<div>
						<p class="text-gray-400 font-light">
							Customer ratings between {{ useDateFormat(form?.rate[0], 'YYYY-MM-D').value }} & {{ useDateFormat(form?.rate[1], 'YYYY-MM-D').value }}
						</p>
						<p>
							<span class="text-3xl font-bold">{{
				...
				<p class="flex items-center space-x-1">
					...
					></span>
					<span class="text-green-600 inline"
					>{{ filteredRatingData?.count }} ratings in total</span
					>
				</p>
			</div>
		</div>
