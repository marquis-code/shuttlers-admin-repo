...
const getUsers = async () => {
	loading.value = true
    const res = await users_api.$_get_users_for_selector_component(search.value) as CustomAxiosResponse
	if (res.type !== 'ERROR') {
...
const asyncFind = async (query: any) => {
    ...
    const result = await users_api.$_get_users_for_selector_component(query, payload) as CustomAxiosResponse
    if (result.type !== 'ERROR') {
        users.value = result.data.data?.length ? result.data.data : []
	}
    loading.value = false
  }
