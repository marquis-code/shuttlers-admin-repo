<template>
	<transition appear name="toast">
		<div
			v-if="active"
			:class="[
				`overflow-hidden max-w-[80%]  shadow-xl fixed top-8 right-4 p-4 py-5 rounded-lg box  bg-white z-[900000000000000]`,
			]"
		>
			<div :style="`width:${BORDER_WIDTH}%`" :class="[`${BG} h-1  absolute  bottom-0 left-0 transition-all duration-300`]" />
			<span @click="closeAlert()">
				<Icon name="close" class="absolute w-5 right-2 top-2 cursor-pointer" />
			</span>

			<div class="flex items-center pr-12 gap-3">
				<Icon :name="ICON" :class="[`${TEXT} w-10`]" />
