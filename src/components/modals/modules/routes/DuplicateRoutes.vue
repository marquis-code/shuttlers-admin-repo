	...
	>
		<div class="space-y-6">
			<div class="field relative">
				<label for="routeCode" class="text-[12px] text-[#6E717C] font-[500] leading-[20px]">New route code</label>
				<input id="routeCode" v-model="routeCreationForm.new_route_code" type="text" class="input-field" required>
			</div>
			<div class="flex items-center gap-x-2">
				<p><label for="copy_itiberaries" class="text-[12px] text-[#6E717C] font-[500] leading-[20px]">Copy Itineraries</label></p>
		...
		<div class="flex justify-between items-center gap-x-6 mt-6">
			<div class="w-full">
				<button class="text-white bg-gray-500 w-full rounded-md py-2.5" @click="useRouteModal().closeRouteDuplicationModal()">
					Close
				</button>
			</div>
			<div class="w-full">
				<button :disabled="!isFormEmpty" :class="[!isFormEmpty ? 'opacity-25 cursor-not-allowed' : null]" class="font-[700] w-full py-2.5 text-white flex justify-center items-center rounded-md bg-[#000005]" @click="handleRouteDuplication">
					<span v-if="!loading" class="flex justify-center items-center gap-2.5">Duplicate Route</span>
					<Spinner v-else />
				</button>
			</div>
		</div>
