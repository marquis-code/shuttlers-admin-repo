<template>
	<Modal
		modal="$atts.modal"
		:title="`${
			Object.keys(selectedVehicleCategory).length ? 'Edit' : 'Configure'
		} Vehicle Category`"
		:no-close-btn="false"
		class="text-center"
	>
		<form
			...
			@submit.prevent="handleCreateVehicleCategory"
		>
			<div>
				<div v-if="form.image">
					<div class="p-4">
						<label for="updatedFile">
							<p
								class="flex justify-center items-center gap-x-2 text-shuttlersGreen cursor-pointer"
							>
								<img
									:src="updatedPreviewImage || form.image"
									alt="previewAmenity"
									class="h-16 w-16 rounded-full border cursor-pointer border-gray-600 object-cover"
								>
								Edit Image
							</p>
							<input
				...
				<div v-else>
					<div v-if="!previewUrl" class="space-y-3">
						<label for="image" class="w-full tracking-wide cursor-pointer">
							<p
								class="flex justify-center items-center gap-x-2 text-shuttlersGreen"
							>
								<img
									src="@/assets/images/default-bus.png"
			...
			<div class="flex justify-between items-center flex-col gap-y-3">
				...
				<div class="w-full space-y-2">
					...
					<input
						id="seat_utilization"
						v-model="form.break_even_utilization"
						type="number"
						class="py-2.5 border outline-none rounded-md px-3 w-full input-field"
					>
					<div class="flex justify-start items-start">
						<span
							v-if="isBreakEvenUtilizationExceeded"
							class="tex text-rose-600 text-sm"
						>Value should be between 0.0 to 1.0</span
						>
					</div>
				</div>
				<div class="w-full">
				...
				<div class="w-full">
					...
					<select
						id="pricing_margin_unit"
						v-model="form.pricing_margin_unit"
						class="py-2.5 border outline-none rounded-md px-3 w-full input-field"
					>
			...
			<div class="flex justify-start items-start">
				<button
					type="button"
					class="rounded-lg px-3 py-2 text-xs border border-gray-600 text-gray-500"
					@click="addRow"
				>
					+ Add City
				</button>
...
const isButtonEnabled = computed(() => {
  return !!(
    form.name &&
    form.description &&
    form.break_even_utilization &&
    form.break_even_utilization &&
    form.pricing_margin &&
    form.pricing_margin_unit
  )
})
...
const form = reactive({
  ...
  break_even_utilization:
    selectedVehicleCategory.value?.break_even_utilization || ('0.0' as any),
  pricing_margin_unit:
    selectedVehicleCategory.value?.pricing_margin_unit || 'percent',
...
const updatedPreviewImage = ref() as any
...
const onEditFileSelected = (e) => {
	...
	if (file) {
		...
		updatedPreviewImage.value = URL.createObjectURL(file)
		fileReader.readAsDataURL(file)
	}
}
...
const isBreakEvenUtilizationExceeded = computed(() => {
  return (
    form.break_even_utilization >= 0.0 && form.break_even_utilization <= 1.0
  )
})
</script>
