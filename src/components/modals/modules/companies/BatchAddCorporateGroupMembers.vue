<template>
	<Modal modal="$atts.modal" title="Add Members">
		<form class="flex flex-col gap-6 space-y-7" @submit.prevent="handleSubmit">
			...
			<div class="flex space-x-6">
				<button type="button" class="bg-gray-500 text-white p-3 rounded-md w-full"
					@click="useCompaniesModal().closeBatchAddCorporateGroupMembers()">
					Cancel
				</button>
				<button type="submit" class="btn-primary w-full" :disabled="!isFormEmpty">
...
const handleSubmit = async () => {
	await createCorporateGroupMembers()
}
