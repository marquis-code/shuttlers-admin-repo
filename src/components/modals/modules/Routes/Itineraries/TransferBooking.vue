<template>
	<Modal modal="$atts.modal" title="Transfer Bookings">
				...
				>
					<template v-if="new_driver?.id" #selection="">
						<div class="flex gap-2 w-full overflow-hidden max-w-[300px]">
							<img
								v-if="new_driver.avatar"
								:src="new_driver.avatar"
							...
							<div class="flex flex-col gap-1">
								...
								<p v-if="new_driver.vehicle?.name" class="text-xs">
									{{ new_driver.vehicle.brand }} {{ new_driver.vehicle.name }}
									{{
										new_driver.vehicle.registration_number
											? `• ${new_driver.vehicle.registration_number}`
											: ""
									}}
									{{
										new_driver.vehicle.seats
											? `• ${new_driver.vehicle.seats} Seater`
											: ""
									}}
								</p>
						...
						<div class="flex gap-2 w-full overflow-hidden max-w-[300px]">
							...
							<div class="flex flex-col gap-1">
								...
								<p v-if="option.vehicle?.name" class="text-xs">
									{{ option.vehicle.brand }} {{ option.vehicle.name }}
									{{
										option.vehicle.registration_number
											? `• ${option.vehicle.registration_number}`
											: ""
									}}
									{{
										option.vehicle.seats
...
import { useItineraryDrivers } from '@/composables/modules/routes/itineraries/driver/fetch'
...
const enableButton = computed(() => {
  return !!Object.keys(new_driver.value).length
})
