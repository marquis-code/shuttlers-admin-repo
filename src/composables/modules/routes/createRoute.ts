...
export const activeRoute = ref({}) as Ref<Record<string, any>>
const showDatePicker = ref(false)

export const legTotalDurationValue = computed(() => {
  let total = 0
...
export const legTotalDurationText = computed(() => {
  return legTotalDurationValue.value
    ? `${Math.floor(legTotalDurationValue.value / 60)} mins`
    : '0 mins'
})
export const legTotalDistanceValue = computed(() => {
  let total = 0
  activeRoute.value?.legs?.forEach((el) => {
...
export const legTotalDistanceText = computed(() => {
  return legTotalDistanceValue.value
    ? `${(legTotalDistanceValue.value / 1000).toFixed(2)} km`
    : '0 km'
})
const center = computed(() => {
  if (polyLine.value.length)
    return polyLine.value[Math.floor(polyLine.value.length / 2)]
...
export const populateFields = (data: Record<string, any>) => {
  ...
  obj.route_availability.value =
    JSON.parse(data?.route_availability_days).length === 7
      ? 'everyday'
      : 'selected_days'
  ...
  obj.avail_start_date.value = data?.route_availability_start_date
    ...
    : ''
  obj.avail_end_date.value = data?.route_availability_end_date
    ? moment(data?.route_availability_end_date).format('YYYY-MM-DD')
    : ''
...
export const useCreateRoute = () => {
  ...
  const createRoute = async () => {
    ...
    const res = (await routes_api.$_create_route(
      payload
