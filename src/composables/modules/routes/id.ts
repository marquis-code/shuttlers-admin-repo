...
export const useRouteTrips = () => {
  ...
  watch(
    [
      ...
      filterData.endDate,
      filterData.itineraryId,
      filterData.tripStatus
    ],
    (val) => {
      getRouteTrips()
    }
  ...
  const getRouteTrips = async () => {
    loading.value = true
    const res = (await routes_api.$_get_route_trips_by_id(
  ...
  const onFilterUpdate = (data) => {
    if (filterData.startDate.value.length || filterData.endDate.value.length) {
...
export const useRoutePaymentOptionsById = () => {
  ...
  const getRoutePaymentOptionsById = async () => {
    ...
    const res = (await routes_api.$_get_payment_options_by_id(
      selectedRouteId.value
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
  ...
  return {
    selectedRoute,
    routePaymentOptionsList,
    loadingSelectedRoutePaymentOptions,
...
export const useTripStartTimeById = () => {
  ...
  return {
    selectedRoute,
    loadingRouteItineraries,
    getTripStartTimeById,
    itineraries
  }
}
...
export const useRoutePassengersById = () => {
  ...
  const getRoutePassengerseById = async (id: string) => {
    ...
    const res = (await routes_api.$_get_route_passengers(
      id
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
...
export const useRouteDriversById = () => {
  ...
  const getRouteDriversById = async (id: string) => {
    ...
    const res = (await routes_api.$_get_route_drivers(
      id
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
...
export const useRouteBookingsById = () => {
  ...
  const getRouteBookingsById = async (id: string) => {
    ...
    const res = (await routes_api.$_get_route_bookings(
      id,
      corporateId.value
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
  ...
  return {
    selectedRoute,
    loadingRouteBookings,
    getRouteBookingsById,
    routeBookings,
    payload,
    corporateId
  }
...
export const useItinerariesByRouteId = () => {
  ...
  const getRouteItinerariesByRouteId = async (id: string) => {
    selectedRouteId.value = id
    loading.value = true
    const res = (await routes_api.$_get_itineraries_by_route_id(
      id
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
...
export const useGetVehicleByItneraryId = () => {
  ...
  const getRouteItineraryVehiclesByItineraryId = async (id: string) => {
    ...
    loading.value = true
    const res =
      (await routes_api.$_get_routes_itineraries_vehicles_by_itinerary_id(
        id
      )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
...
export const useBusstopsByItineraryId = () => {
  ...
  const getBusstopsByItineraryId = async (id: string) => {
    loading.value = true
    const res = (await routes_api.$_get_busstops_by_itinerary_id(
      id
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
...
export const useRoutePricingByItineraryId = () => {
  ...
  const getRoutePricingInformation = async (id: string) => {
    loading.value = true
    const res = (await routes_api.$_get_route_pricing(
      id,
      convertObjWithRefToObj(routePricingData)
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
...
export const useRouteBusstopsByRouteId = () => {
  ...
  const getRouteBustopsById = async (id: string) => {
    selectedRouteId.value = id
    loading.value = true
    const res = (await routes_api.$_get_route_busstops_by_id(
      id
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
...
export const useRouteUpcomingTripsPassengersByTripId = () => {
  ...
  const getRouteTripsPassengersById = async (id: string) => {
    selectedRouteId.value = id
    loading.value = true
    const res = (await routes_api.$_get_route_passengers_by_route_id(
      id
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
