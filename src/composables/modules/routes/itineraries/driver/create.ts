...
export const useAddDriver = () => {
  const addDriver = async () => {
    ...
    loading.value = true
    const res = (await drivers_api.$_assign_driver_to_route(
      obj.driver.value!.id,
      payload
  ...
  const getVehicleCostOfSupply = async () => {
    ...
    if (!vehicle_id) {
      useAlert().openAlert({
        type: 'Alert',
        msg: 'This driver has no vehicle'
      })
      return
    }
    loading_cost_of_supply.value = true
    const res = (await routes_api.$_get_vehicle_cost_of_supply(
      useRoute().params.iti_id as string,
      vehicle_id
    )) as CustomAxiosResponse
    if (res.type !== 'ERROR') {
      obj.cost_of_supply.value = res.data?.cost_of_supply
        ? res.data.cost_of_supply
        : 0
    }
